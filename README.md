...
雀荘と麻雀プロをマッチングするためのWebアプリケーションです。  
既存の成績管理機能に加えて、マッチング機能と管理画面を実装しています。
...
### 画面一覧
1. **マッチングダッシュボード** - ロール別サマリー表示
2. **相手検索画面** - 雀荘/麻雀プロの検索とオファー送信
3. **オファー管理画面** - 承認待ち/やり取り中/完了の管理
4. **メッセージ画面** - 成立オファーのチャット
5. **管理画面** - 全体データの監視（管理者のみ）
...
#### マッチング
- `GET /api/matching/users` - 相手ユーザー検索
- `POST /api/matching/offers` - オファー送信
- `GET /api/matching/offers` - オファー一覧取得
- `PATCH /api/matching/offers/{id}/status` - オファーステータス更新
- `GET /api/matching/offers/{id}/messages` - メッセージ一覧取得
- `POST /api/matching/offers/{id}/messages` - メッセージ送信

#### 管理画面
- `GET /api/admin/overview` - 全体サマリー
- `GET /api/admin/users` - 会員一覧
- `GET /api/admin/offers` - オファー一覧
